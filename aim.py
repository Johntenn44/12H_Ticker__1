...
async def send_telegram_message_async(message: str, chat_id_override: Optional[str] = None):
    """Send telegram message asynchronously using aiohttp"""
...
async def create_exchange() -> ccxt.Exchange:
    """Create async CCXT exchange instance"""
...
# Global collections for ML training samples
ml_samples_buffer = deque(maxlen=5000)  # Increased buffer size
...
def add_ml_sample(signals, price_move):
    """Add ML sample to buffer instead of immediate lists"""
...
def train_ml_model():
    """Train ML model using buffered samples - called once per scan cycle"""
    global ml_trained, calibrated_ml_model
    
    with ml_lock:
        if len(ml_samples_buffer) < len(INDICATOR_FUNCTIONS) * 10:  # Minimum samples needed
            ml_trained = False
            calibrated_ml_model = None
            return
...
def adaptive_signal(df):
    ...
    # Fallback to weighted indicators
    up_w = down_w = total = 0
...
def backtest_update(df):
    """Updated backtest function that adds samples to buffer instead of training immediately"""
    start = max(50, 0)
    if len(df) < start + 2:
        return
    ...
    for i in range(start, len(df) - 1):
        ...
        # Add sample to buffer (not immediate training)
...
async def fetch_latest_ohlcv_async(exchange: ccxt.Exchange, symbol: str, timeframe: str = '6h', limit: int = 500):
    """Fetch OHLCV data asynchronously with 6h timeframe"""
    try:
...
# --- Position Sizing for $1 max trade with 10x leverage (adjusted for 6h) ---
MAX_UNLEVERAGED_NOTIONAL = 1.0  # $1 max notional per trade
LEVERAGE = 10  # 10x leverage (adjusted from 15x for 6h candles: 15 * 4/6 = 10)

def calculate_position_size_dollars(price: float) -> Tuple[float, float]:
    """Calculate position size allowing fractional units"""
    units = MAX_UNLEVERAGED_NOTIONAL / price
    notional = units * price
    return notional, units

# --- Process a Single Symbol (for multiprocessing) ---
def process_symbol_worker(symbol_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """Worker function for multiprocessing - processes a single symbol"""
    try:
        symbol = symbol_data['symbol']
        df_data = symbol_data['df_data']
        
        # Recreate DataFrame from serialized data
        ...
        # Backtest update (adds samples to buffer)
        sub = df.iloc[-500:] if len(df) > 500 else df
        if len(sub) > 50:
            backtest_update(sub)
...
def seconds_until_next_6h_utc():
    """Calculate seconds until next 6h UTC interval"""
    now = datetime.utcnow()
...
async def async_main_scan():
    """Main async function that handles data fetching and processing"""
    try:
        ...
        try:
            ...
            for symbol in CRYPTO_SYMBOLS:
                task = fetch_latest_ohlcv_async(exchange, symbol, timeframe='6h', limit=500)
            ...
            # Train ML model once per scan cycle (after all processing)
...
# --- Main Loop ---
def main():
    """Main function that runs the scanning loop"""
    try:
        while True:
            # Run async scan
            asyncio.run(async_main_scan())
            
            # Calculate sleep time for next 6h UTC
            sleep_time = seconds_until_next_6h_utc()
            print(f"Sleeping {sleep_time:.0f}s until next 6h UTC.")
